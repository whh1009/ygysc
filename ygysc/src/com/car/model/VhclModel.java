...
public class VhclModel extends Model<VhclModel> {
	...
	public Page<Record> getStarCarList(int pageNumber, String sqlCondition) {
		log.warn("明星车源列表sql：select v.vhclId, v.modelName, v.displayPrice, v.productYear, v.tableShowMileage, v.vhclRegProv, v.vhclAddrProv, v.authNo, p.picType,p.vhclPic from Vhcl v, Pic p where v.vhclId=p.vhclId  and p.picType = (select min(picType) from Pic sp where sp.vhclId = v.vhclId)"+sqlCondition+" order by v.vhclId ");
		return Db.paginate(pageNumber, Constrant.PAGE_SIZE, "select v.vhclId, v.modelName, v.displayPrice, v.productYear, v.tableShowMileage, v.vhclRegProv, v.vhclAddrProv, v.authNo, p.picType,p.vhclPic ", " from Vhcl v, Pic p where v.vhclId=p.vhclId  and p.picType = (select min(picType) from Pic sp where sp.vhclId = v.vhclId)"+sqlCondition+" order by v.vhclId ");
	}
